...
<div  class='editor_bottom2'>";
    ...
    public function typeid($info,$value)
    {
        ...
        $id = $field = $info['field'];
        $value = $value ? $value : $this->data[$field];
        $parentid = explode(',',$info['setup']['default']);
        foreach ($parentid as $key => $value) {
            $keyid[] = $types[$value]['keyid'];
        }

        $options = array();
        foreach((array)$types as $key => $r) {
            if($r['lang'] == LANG_ID){
                if(!in_array($r['keyid'], $keyid) || in_array($r['typeid'], $keyid))
                    continue;
                $r['id'] = $r['typeid'];
                $array[] = $r;
                $options[$key] = $r['name'];
        ...
        if($fun=='select'){
            ...
            $newinfo['options'] = $options;
            return $this->$fun($newinfo,$typeid);
        }
    }
    ...
    public function images($info,$value)
    {
        ...
        $parseStr['yzh_auth'] = get_yzh_auth(10,$info['setup']['upload_maxsize'].'MB',$info['modelid']);
...
';
    ...
    //关联栏目
    public function cat_relation($info,$value)
    {
        $info['setup'] = is_array($info['setup']) ? $info['setup'] : json_decode($info['setup'], true);
        $parentid = !empty($info['setup']['parentid']) ? $info['setup']['parentid']:$info['parentid'];
        $id = $field = $info['field'];
        $value = !empty($value) ? $value : $this->data[$field];
        ...
        foreach($list as $cats){
            $str .= '<li id="'.$info['field'].'_'.$cats['id'].'">·<span>'.$cats['catname'].'</span><a href="javascript:;" class="close" onclick="remove_relation(\''.$info['field'].'\','.$cats['id'].')"></a>
            <input type="hidden" name="'.$info['field'].'[]" value="'.$cats['id'].'"/>
            </li>';
        }
        $parseStr='<input type="hidden" name="'.$info['field'].'" id="cat_relation" value="" style="50">
                        <ul class="list-dot" id="relation_text_cat">
                        '.$str.'
                        </ul>
                        <input type="button" value="添加相关栏目" onclick="openwin(\'/?a=public_cat_relationlist&m=Content&g=admin&parentid='.$parentid.'&field='.$info['field'].'\',\'添加相关栏目信息\',\'800px\',\'550px\')" class="btn">
                        <span class="edit_content">
                    </span>';
        return $parseStr;
    }
