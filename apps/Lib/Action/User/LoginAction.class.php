            ...
            if ($data = $member_db->create()) {
                ...
                if (false !== $member_db->add()) {
                    ...
                    if($this->MemberConfig['emailcheck']){
                        $auth = authcode($uid."-".$username."-".$email, 'ENCODE', C('ADMIN_ACCESS'), 3600*24*3);//3天有效期
                        $url = 'http://'.$_SERVER['HTTP_HOST'].'/user/login/regcheckemail?code='.$auth;

                        $message = str_replace(array('{username}','{url}','{sitename}'),array($username,$url,$this->Config['site_name']),$this->MemberConfig['emailchecktpl']);

                        $r = sendmail($email,'['.$this->Config['site_name'].']注册认证邮件', $message, $this->SysConfig);
                        if ($r) {
