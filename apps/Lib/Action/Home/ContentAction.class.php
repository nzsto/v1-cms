...
class ContentAction extends PublicAction
{
        ...
        if ($modelname == 'Page') {
            ...
            foreach($data as $key=>$c_d){
                if (!empty($fields[$key])){
                    ...
                    if ($fields[$key]['type'] == 'cat_relation') {
                        //栏目关联信息
                        if (!empty($data['cat_relation'])) {
                            $temp_modelname = 'Category';
                            $data['cat_relation'] = json_decode($data['cat_relation'], true);
                            $cat_relation = M($temp_modelname)->where(array('id'=>array('in',$data['cat_relation']),'lang'=>array('eq',LANG_ID)))->select();
                            M($temp_modelname)->getLastSql();
                        }
                    }
                }
            }
            $data['cat_relation'] = $cat_relation;
    ...
    public function detail()
    {
        ...
        if($listorder!=0){
            ...
            $prea = $model_db->field('title,url,thumb')->where($map)->order('listorder desc')->limit('1')->select();

            $map['listorder'] = array('gt',$listorder);
            $next = $model_db->field('title,url,thumb')->where($map)->order('listorder asc')->limit('1')->select();
        } else{
            $map['id'] = array('gt',$id);
            $prea = $model_db->field('title,url,thumb')->where($map)->order('id asc')->limit('1')->select();
            $map['id'] = array('lt',$id);
            $next = $model_db->field('title,url,thumb')->where($map)->order('id desc')->limit('1')->select();
        }
            ...
            if (!empty($fields[$key])){
                ...
                if ($fields[$key]['type'] == 'cat_relation') {
                    ...
                    if (!empty($data['cat_relation'])) {
                        ...
                        $cat_relation = M($temp_modelname)->where(array('id'=>array('in',$data['cat_relation']),'lang'=>array('eq',LANG_ID)))->select();
                        M($temp_modelname)->getLastSql();
                        $this->assign('cat_relation',$cat_relation);
                    }
                }
