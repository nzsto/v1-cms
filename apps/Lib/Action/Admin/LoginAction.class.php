...
class LoginAction extends Action
{
    ...
    public function doLogin()
    {
        ...
        require_once "./public/verifycode/clicaptcha.class.php";
        $clicaptcha = new clicaptcha();
        if (!($clicaptcha->check($_POST['clicaptcha-submit-info']))) {
            $this->error('验证码错误，请重新输入！');
            exit;
        }

        if (empty($username)) {
            $this->error('账号不能为空，请输入正确的用户名！');
            exit;
        }elseif (empty($password)){
            $this->error('密码不能为空，请输入正确的密码！');
            exit;
        }
        //旧版验证码验证 
