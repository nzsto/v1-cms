...
class ContentAction extends PublicAction
{
    ...
    public function cancel_push()
    {
        ...
        foreach($data as $key=>$r){
            $model_db->save(array("id"=>$r['id'],"posid"=>""));
        }
        $this->success('取消推送！');
    }
    ...
    //相关栏目选择
    public function public_cat_relationlist() {
        $parentid = I('get.parentid', 0, 'intval');
        $cat_db = M('category');
        $map = array();
        $map['lang'] = LANG_ID;
        if(!empty($parentid)){
            $cats = getCategory($parentid);
            $map['id'] = array('IN',$cats['arrchildid']);
        }
        $count = $cat_db->where($map)->count();
        import("@.ORG.Page");
        $page = new Page($count, 10);
        $data = $cat_db->where($map)->limit($page->firstRow . ',' . $page->listRows)->order(array('id' => "ASC"))->select();

        $this->assign('cat_relationlist',1);
        $this->assign('data', $data);
        $this->assign('Page', $page->show());
        $this->assign('modelid', $modelid);
        $this->assign('field', $_GET['field']);
