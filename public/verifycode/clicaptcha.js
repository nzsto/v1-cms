	...
	$.fn.extend({
		'clicaptcha': function(options){
			...
			if(!$('#clicaptcha-box').length){
				$("body").prepend('<i style="top:0px;left:0px;" id="number1" class="number">1</i>'+
					'<i style="top:0px;left:0px;" id="number2" class="number">2</i>'+
					'<i style="top:0px;left:0px;" id="number3" class="number">3</i>'+
					'<i style="top:0px;left:0px;" id="number4" class="number">4</i>');
				$('.code-box').append('<div id="clicaptcha-box">'+'<div class="clicaptcha-title"></div>'
					+'<img class="small-img" src="" />'+
					'<img class="clicaptcha-img" id="bigImg" src="" alt="验证码加载失败，请点击刷新">'+
					'<div class="clicaptcha-refresh-box">'+
						'<div class="clicaptcha-refresh-line clicaptcha-refresh-line-left"></div>'+
						'<a href="javascript:;" class="clicaptcha-refresh-btn" title="刷新">刷新图片</a>'+
			...
			$('#clicaptcha-box .clicaptcha-img').attr('src', opts.src + '?type=small&&' + new Date().getTime()).load(function(){
				$('#clicaptcha-box .small-img').attr('src', "/public/verifycode/clicaptcha.php" + '?' + new Date().getTime());
				var thisObj = $(this);
				var text = Cookies.get('clicaptcha_text').split(',');
				var title = '请按顺序点击图中文字';
						...
						if(xyArr.length == text.length){
							...
							$.ajax({
								...
								if(cb == 1){
									...
									setTimeout(function(){
										$("#number1").attr('style','none');
										$("#number2").attr('style','none');
										$("#number3").attr('style','none');
										$("#number4").attr('style','none');
